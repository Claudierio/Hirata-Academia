<template>
  <div class="main-container">
    ...
    <v-data-table :headers="headers" :items="desserts" sort-by="calories" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Atualizar Professor Estagiario</v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-spacer></v-spacer>
          <v-dialog v-model="dialog" max-width="500px">
            <template v-slot:activator="{ on, attrs }">
              <v-btn color="primary" dark class="mb-2" v-bind="attrs" @click="CadastrarProfEstagiario" v-on="on">
                Cadastrar Novo Professor Estagiario
              </v-btn>
            </template>
            <v-card>
              <v-card-title>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      ProfessorEstagiarioService.deleteById(this.desserts[this.editedIndex].id).then(
        response => {
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        ProfessorEstagiarioService.update(this.desserts[this.editedIndex].id, this.editedItem).then(
          response => {
