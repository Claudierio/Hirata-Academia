<template>
  <div class="main-container">
    ...
    <v-data-table :headers="headers" :items="desserts" sort-by="calories" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Atualizar Professor</v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-spacer></v-spacer>
          <v-dialog v-model="dialog" max-width="500px">
            <template v-slot:activator="{ on, attrs }">
              <v-btn color="primary" dark class="mb-2" v-bind="attrs" @click="CadastrarProfessor" v-on="on">
          ...
          <v-dialog v-model="dialogDelete" max-width="530px">
            <v-card>
              <v-card-title class="text-h9">Você tem certeza que deseja remover esse Professor?</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      ProfessorService.deleteById(this.desserts[this.editedIndex].id).then(
        response => {
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        ProfessorService.update(this.desserts[this.editedIndex].id, this.editedItem).then(
          response => {
